...
/**
  ...
  *   routing register ICRx and can be from RI_InputCaptureRouting_0 to RI_InputCaptureRouting_22.
  * @retval None.
  */
                                 ...
                                 FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    if (AnalogSwitchRegister == (uint8_t) 0x10)
    {
      ...
      RI->ASCR1 |= (uint8_t) ((uint8_t)1 <<  AnalogSwitchIndex);
    }
    else
    {
      /* Enable the analog switch */
      RI->ASCR2 |= (uint8_t) ((uint8_t)1 <<  AnalogSwitchIndex);
    }
  }
  else
  {
...
void SYSCFG_REMAPPinConfig(REMAP_Pin_TypeDef REMAP_Pin, FunctionalState NewState)
{
  ...
  /* Check if REMAP_Pin in RMPCR1 register */
  if (regindex == 0x01)
  {
  ...
  /* REMAP_Pin is in RMPCR2 register */
  else
  {
    if (NewState != DISABLE)
    {
