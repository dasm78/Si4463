...
/**
  ...
  *   Address (if data is from/to  peripheral) or DMA channelx Memory1 Base Address
  *   (if data is from Memory0 to Memory1).
  ...
  * @param  DMA_MemoryIncMode : Specifies the DMA channelx memory Incrementation/Decrementation
  *   mode, this parameter can be a value of @ref DMA_MemoryIncMode_TypeDef.
...
/**
  ...
  * @param  DMA_Channelx : selects the DMA Channelx where x can be 0 to 3
  * @param  NewState: new state of the DMA Channelx. This parameter can be: ENABLE or DISABLE.
  * @retval None
...
FlagStatus DMA_GetFlagStatus(DMA_FLAG_TypeDef DMA_FLAG)
{
  ...
  if (((uint16_t)DMA_FLAG & 0x0F00) != (uint16_t)RESET)
  {
    ...
    bitstatus = (FlagStatus)(DMA_Channelx->CSPR & (uint16_t)DMA_FLAG);
  }
  else if (((uint16_t)DMA_FLAG & 0x1000) != (uint16_t)RESET)
  {
    bitstatus = (FlagStatus)(tmpgir1 & (uint8_t)DMA_FLAG);
  }
  else /*if ((DMA_FLAG & DMA_FLAG_GB) != (uint16_t)RESET)*/
  {
    bitstatus = (FlagStatus)(tmpgcsr & DMA_GCSR_GB);
  }
...
/**
  * @brief  Clears the DMA Channels' pending flags.
  * @param  DMA_FLAG: specifies the flag to clear.
  *   This parameter can be a value of (or a combination for the same DMA channel)
  *   of @ref DMA_FLAG_TypeDef
  * @retval None
  */
...
ITStatus DMA_GetITStatus(DMA_IT_TypeDef DMA_IT)
{
  ...
  else if  ((DMA_IT & 0x20) != (uint8_t)RESET)
  {
    DMA_Channelx = DMA1_Channel1;
  }
  else if  ((DMA_IT & 0x40) != (uint8_t)RESET)
  {
    DMA_Channelx = DMA1_Channel2;
  }
  else
  ...
  bitstatus = (ITStatus)((uint8_t)tmpreg & (uint8_t)tmp2);
