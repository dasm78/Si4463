...
void LCD_DeInit(void)
{
  ...
  for (counter = 0;counter < 0x04; counter++)
  {
...
/**
  ...
  *   The pulses on duration have a sense only when enabling the High drive
  */
void LCD_PulseOnDurationConfig(LCD_PulseOnDuration_TypeDef LCD_PulseOnDuration)
{
  ...
  LCD->CR2 |= (uint8_t)LCD_PulseOnDuration;
}
/**
...
void LCD_ContrastConfig(LCD_Contrast_TypeDef LCD_Contrast)
{
  ...
  LCD->CR2 |= (uint8_t)LCD_Contrast; /* Select the maximum voltage value Vlcd */
}

/**
  * @brief  Configures the LCD Port Mask.
  * @param  LCD_PortMaskRegister: The LCD register index in the port mask matrix.
  *   This parameter can be any of the @ref LCD_PortMaskRegister_TypeDef enumeration.
